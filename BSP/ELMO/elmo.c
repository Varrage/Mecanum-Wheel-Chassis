...
//		SLIDEWAY_ELMO_SAVE_Y[SLIDEWAYy_ELMO_SAVE_count_Y].speed=speed;
...
void CAN1_RX0_IRQHandler()
{
	...
	{
		Elmo_Init_Flag |= (1<<((RxMessage.StdId&0x0F)-1));//是1右移多少位
		memcpy(&MotorPos[RxMessage.StdId - COBID_TPDO2-1],&RxMessage.Data[4],4);
	}
		//	Elmo_Init_Flag |= (1<<((RxMessage.StdId&0x0F)-1));//是1右移多少位
//	if((RxMessage.StdId>COBID_TPDO2) && (RxMessage.StdId<(COBID_TPDO2 + 17)))
//		memcpy(&MotorPos[RxMessage.StdId - COBID_TPDO2-1],&RxMessage.Data[4],4);

//	if(RxMessage.StdId>=0x1FF&&(RxMessage.StdId<0x201))
//	{
//		M2006_Get_Feedback(RxMessage.StdId,RxMessage.Data);
//		pos_rec(RxMessage.StdId - 0x200);
//	}
	if(RxMessage.StdId>=0x199&&(RxMessage.StdId<0x208))
	{
		M2006_Get_Feedback(RxMessage.StdId,RxMessage.Data);
		pos_rec(RxMessage.StdId - 0x200);
	}
...
********************************************************************************
  ...
  *@  input     : None
  *@  output    : None
                如果CAN无法向elmo发送信号，则CAN_Error=1，关闭TIM中断，返回值0x80000000
                如果某一个elmo没有向主控传回信息，返回值为该elmo的编号
                如果正常工作，返回0
********************************************************************************
*/
int Self_test(void)
...
********************************************************************************
  ...
  *@  output    : None
                  如果刚发完速度指令就读取编码器，第一次的读数不准，之后就好了
********************************************************************************
*/
